# Source: https://github.com/qinglew/PointCloudTransformer/blob/main/model.py

import torch
import torch.nn as nn
import torch.nn.functional as F

from src.models.baselines.pct.module import (
    Embedding, NeighborEmbedding, OA, SA
)
...
class PCT(nn.Module):
    def __init__(self, samples=[512, 256]):
        ...
        num_layers = 4
        final_dim = samples[1] * (num_layers + 1)
        self.linear = nn.Sequential(
            nn.Conv1d(final_dim, 1024, kernel_size=1, bias=False),
            nn.BatchNorm1d(1024),
            nn.LeakyReLU(negative_slope=0.2)
        )
...
# Head for classification with 1100 categories
class Classification(nn.Module):
    def __init__(self, num_categories=20):
        super().__init__()
        self.linear1 = nn.Linear(1024, 1024)
        self.linear2 = nn.Linear(1024, num_categories)
